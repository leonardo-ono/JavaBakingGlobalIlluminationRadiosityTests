...
//        colorp3 = 1;
        ...
        if (texture != null) {
            ...
            double nt = t - (int) t;
            if (nt < 0) nt += 1.0;
            double txd = (ns * (texture.getWidth() - 1));
            int tx = (int) txd;
            double txp = txd - tx;
            
            int textureHeight = texture.getHeight() - 1;
            double tyd = textureHeight - (nt * textureHeight);
            int ty = (int) tyd;
            double typ = tyd - ty;

            /*
            texture.getPixel(tx, ty, color2);
            texture.getPixel(tx + 1, ty, color3);
            lerp(color2, color3, color4, txp);

            texture.getPixel(tx, ty + 1, color5);
            texture.getPixel(tx + 1, ty + 1, color6);
            lerp(color5, color6, color7, txp);

            lerp(color4, color7, color, typ);
            */
